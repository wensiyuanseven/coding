<template>
  <!-- v-if的作用是有数据才进行渲染 -->
  <el-tree :data="allData" v-if="allData.length"
   default-expand-all
   :expand-on-click-node="false"
   :render-content="renderEvent"></el-tree>
</template>
<script>
...
export default {
    ...
    watch: { //监听数据变化，因为在子组件渲染完成时，数据可能还未更新，那么我们就需要对其进行监听并更新
      data() { // 数据更新了就重新进行渲染
          this.transferData();
      }
    },
    methods: {
                ...
                <div style={{width:'50%'}}>
                    {
                        ...
                        :
                        <i class="el-icon-folder-document"></i>
                    }
                    {/**进行重命名的输入框*/
                      data.id === this.currentId ?
                        <el-input value={this.currentContent}
                        on-input={this.handleInput} /**受控组件，如果想可以输入，就需要添加这个 */
                        ></el-input>
                      : data.name
                    }
                    { /**操作时隐藏下拉按钮 */
                     data.id === this.currentId ?
                        //  操作按钮
                        <span >
                            <el-button type="text" on-click={this.sureEvent.bind(this, data)}>确认</el-button>
                            <el-button type="text" on-click={this.cancelEvent}>取消</el-button>
                        </span>
                     :
                        ...
                        >
                            ...
                            {
                                dropList.map(item=> <el-dropdown-item command={item.text} /**绑定的触发值*/>{item.value}</el-dropdown-item>)
                            }
                            </el-dropdown-menu>
                        </el-dropdown>
                    }
        ...
        isParent(data) { // 判断是不是父文件夹
            return data.type === 'parent';
        },
        handleCommand(data, value) { //点击下拉菜单项触发的事件 value其实就是我们当前触发的item.text的值
        console.log(111)
            if(value === 'rn') {
