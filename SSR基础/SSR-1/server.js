...
router.get("/", async ctx => {
  // 此时打包之后的结果全是字符串，字符串没有任何的事件功能
  // 所以js逻辑不生效(打包之后vue-server-renderer也会丢弃js逻辑)，
  // 所以页面中只有css(vue-style-loader的功劳和html逻辑)
  // 所以需要把客户端打包后的结果挂载到模板上，因为客户端代码包含事件逻辑
  // 此时需要三个个步骤 1.在模板(dist目录)中先手动引入客户端打包后的结果
  // <script src="./client.bundle.js"></script> (因为js中的事件逻辑不需要被爬虫爬取)
  // 2.但此时服务器并没有读取此文件，所以需要 koa-static 静态服务中间件 去dist查找此文件
  // 3.需要在App.vue中指定id='app' ,俗称-客户端激活
  // 此方法比较笨
