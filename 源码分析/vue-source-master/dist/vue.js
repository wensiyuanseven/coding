(function (global, factory) {
  ...
  var LIFECYCLE_HOOKS = ['beforeCreate', 'created', 'mounted', 'beforeMount', 'beforeUpdate', 'updated'];
  var strats = {};
  ...
  methods.forEach(function (method) {
    ...
    arrayMethods[method] = function () {
      ...
      var ob = this.__ob__; // this指数组本身 this指向arrayMethods  arrayMethods 又指向data  data等于数组

      for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      var result = oldArrayMethods[method].apply(this, args); // let result  = oldArrayMethods[method].call(this,...args);
  ...
  function defineReactive(data, key, value) {
    var childOb = observe(value); // 如果传入的值还是一个对象的话 就做递归循环检测   初始监听时都是null  因为不是对象
  ...
  function compileToFunctions(template) {
    ...
    console.log(render, "render"); // 注释节点 自闭和标签 事件绑定 @click  class slot插槽
  ...
  function nextTick(fn) {
    callbacks.push(fn); // true  事件环的概念 promise mutationObserver setImmediate setTimeout
